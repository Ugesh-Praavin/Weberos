...
const ServiceCard = ({ title, description, price, features, popular, icon }: ServiceCardProps) => {
  return (
    <Card className={`hover-grow overflow-hidden border-roadmap-black bg-black/70 backdrop-blur-sm 
      ...
      <CardHeader className={`${popular ? 'bg-roadmap-orange/10' : ''}`}>
        ...
        <CardDescription className="text-center text-roadmap-orange/70">{description}</CardDescription>
      </CardHeader>
        ...
        <div className="mt-6 space-y-3">
          {features.map((feature, index) => (
            <div key={index} className="flex items-start">
              ...
              <span className="text-sm text-roadmap-orange/80">{feature}</span>
            </div>
          ))}
        </div>
      </CardContent>
      <CardFooter>
        <Button className={`w-full ${popular ? 'bg-roadmap-orange hover:bg-roadmap-orange/90 text-black' : 'bg-black hover:bg-black/80 text-roadmap-orange border border-roadmap-orange/30'}`}>
          Select This Package
        </Button>
      </CardFooter>
    </Card>
